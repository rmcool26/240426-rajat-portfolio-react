    ...
    >
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <h3
            className={`font-display text-2xl font-bold leading-tight tracking-tight transition-colors md:text-3xl lg:text-2xl ${
...
export const CaseStudyAccordion = ({ pvnxtCases }: { pvnxtCases: Project[] }) => {
  const [openSlug, setOpenSlug] = useState<string | null>(pvnxtCases[0]?.slug ?? null);

  return (
    <div className="mt-12 border-t border-border md:mt-16">
      {pvnxtCases.map((project, index) => (
        <Row
          key={project.slug}
          item={project}
          index={index}
